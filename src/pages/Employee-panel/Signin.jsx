...
import { login } from "../../utils/employeeLogin";
...
export default function Login() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (res) {
      try {
        const employeeRef = doc(getFirestore(), "employees", res.id);
        ...
        if (querySnapshot.empty) {
          // No login found for today
          await addDoc(loginHistoryRef, {
            timestamp: serverTimestamp(),
            date: todayDateString, // Store the date string for easy querying
          });
          toast.success("Login timestamp recorded");
        } else {
          toast.success("Login timestamp already recorded");
        }
      } catch (error) {
        toast.error("Error recording login timestamp");
      }
  ...
  useEffect(() => {
    console.log("User contecxt: ", user);
  }, [user]);
